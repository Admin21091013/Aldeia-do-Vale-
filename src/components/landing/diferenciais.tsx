...
export default function Diferenciais() {
  return (
    <section id="diferenciais" className="bg-secondary">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
          {diferenciais.map((item, index) => (
            <Card key={index} className="transform border-0 text-center shadow-lg transition-transform duration-300 hover:-translate-y-2">
              <CardHeader className="items-center">
                {item.icon}
                <CardTitle className="pt-4 font-headline text-2xl">{item.title}</CardTitle>
              </CardHeader>
              <CardContent>
